...
description: "Step-by-step guide for uploading point cloud snapshots"
---

<Warning>
**Implementation Notice:** This is a conceptual workflow guide. Actual integrations will receive dedicated, customized endpoints and workflows tailored to specific requirements. Contact Sodex to discuss your integration needs and receive your dedicated API endpoints.
</Warning>

## Complete Upload Workflow

This guide walks you through the complete process of uploading a point cloud snapshot to the platform. The workflow consists of five sequential steps that must be completed in order.
  ...
  "data": [
    {
      ...
      "name": "Point Cloud Integration Project"
    }
  ],
  "pagination": {
    "page": 1,
...
<CodeGroup>
```bash cURL
curl -X POST "https://api.sodex.cloud/v2/projects/{project_id}/snapshots" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
response = requests.post(
    f'https://api.sodex.cloud/v2/projects/{project_id}/snapshots',
    headers=headers,
    json=snapshot_data
)
...
<CodeGroup>
```bash cURL
curl -X GET "https://api.sodex.cloud/v2/snapshots/{snapshot_id}/s3-upload-credentials" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json"
```
...
```python Python
response = requests.get(
    f'https://api.sodex.cloud/v2/snapshots/{snapshot_id}/s3-upload-credentials',
    headers=headers
)
...
```json
{
  "data": {
    ...
    "s3_prefix": "uploads/507f1f77bcf86cd799439022/"
  }
}
```
...
<CodeGroup>
```bash cURL
curl -X POST "https://api.sodex.cloud/v2/snapshots/{snapshot_id}/commit" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "laz_s3_key": "uploads/507f1f77bcf86cd799439022/pointcloud_data.laz"
  }'
```
...
response = requests.post(
    f'https://api.sodex.cloud/v2/snapshots/{snapshot_id}/commit',
    headers=headers,
    json=commit_data
)
