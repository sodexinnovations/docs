...
description: "Complete reference for third-party integration snapshot upload API endpoints"
---

<Warning>
**Third-Party Integration Endpoints:** The endpoints described here are specifically for third-party integrations. Each integration partner will receive a unique integration prefix that is prepended to all API endpoints. Sodex will provide you with your specific integration endpoints and interactive documentation.
</Warning>
...
<Note>
For complete API reference including request/response schemas, Sodex will provide you with integration-specific documentation and interactive testing capabilities.
</Note>
...
```http
GET /{integration-prefix}/organizations/me/projects
```
...
```http
GET /{integration-prefix}/projects/{project_id}
```
...
```http
GET /{integration-prefix}/projects/{project_id}/snapshots-v2
```
...
```http
POST /{integration-prefix}/projects/{project_id}/snapshots
```
...
<ParamField body="location" type="object">
  Geographic location of the snapshot capture in degrees (optional)
...
```http
GET /{integration-prefix}/snapshots/{snapshot_id}
```
...
```http
GET /{integration-prefix}/snapshots/{snapshot_id}/s3-upload-credentials
```
...
```http
POST /{integration-prefix}/snapshots/{snapshot_id}/commit
```
...
<AccordionGroup>
  ...
  <Accordion title="400 - Bad Request">
    ...
    <Note>
    The exact error response format will depend on your specific integration configuration. Sodex will provide you with the exact error format for your integration.
    </Note>
  </Accordion>
...
<Note>
Rate limiting details and specific limits are available in the main API documentation provided by Sodex for your integration.
</Note>
